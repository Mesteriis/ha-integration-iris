...
async def async_setup_entry(
    ...
    entry.async_on_unload(
        entry.runtime_data.entity_sync.register_platform(
            ...
            factory=lambda definition: IrisCatalogButtonEntity(entry, definition),
        )
    )


class IrisCatalogButtonEntity(ButtonEntity):
    ...
    async def async_press(self) -> None:
        if self._command_key is None:
            raise HomeAssistantError(f"IRIS button entity '{self.entity_key}' is missing command_key.")
        await self._command_bus.async_execute(command=self._command_key, payload={})
